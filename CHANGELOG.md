...
## 3.4.1 - 2026-09-01 [EXPERIMENTAL]
- Found (via live testing right after 3.4.0's release) and worked around a server-side limit, undocumented by the API: with `append` (merge all tables into one file) enabled, the extraction service rejects the request with `HTTP 500` beyond 58 selected tables. Reproduced reliably: 58 tables succeed regardless of which one is excluded to get there (tested dropping two different tables), 59 always fails, and 59 tables *without* merging succeed fine — pinning this down to a limit on the number of merged relations, not an issue tied to a specific table's content. BD TOPO® tous thèmes has exactly 59 tables, so "select all" + merge (a natural combination) reliably hit this. The plugin now blocks submission client-side with a clear explanatory message in that case, instead of letting the server's opaque 500 error through
